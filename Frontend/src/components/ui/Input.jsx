...
export const Input = ({ icon: Icon, className = '', ...props }) => (
  <div className="relative w-full">
    ...
    <input
      className={`h-10 w-full rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/50 text-sm text-slate-850 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:border-blue-500 dark:focus:border-blue-500 focus:bg-white dark:focus:bg-zinc-950/70 focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all font-semibold ${
        Icon ? 'pl-9 pr-4' : 'px-3'
      } ${className}`}
      {...props}
    />
...
export const Select = ({ className = '', children, ...props }) => (
  <select
    className={`h-10 w-full rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/50 px-3 text-sm text-slate-850 dark:text-zinc-100 focus:border-blue-500 dark:focus:border-blue-500 focus:bg-white dark:focus:bg-zinc-950/70 focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all cursor-pointer font-semibold ${className}`}
    {...props}
  >
    {children}
  </select>
