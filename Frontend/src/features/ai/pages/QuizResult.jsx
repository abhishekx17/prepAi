      ...
      <div className="max-w-3xl mx-auto space-y-8 select-none">
          ...
          <div className="sm:col-span-4 flex flex-col items-center justify-center">
            <div className="relative w-32 h-32 flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-2xl font-bold tracking-tight text-zinc-50">{percentage}%</span>
        ...
        <section className="space-y-5">
          ...
          {questions.map((q, idx) => {
            ...
            return (
              <div key={idx} className="border border-zinc-800 bg-zinc-900/10 rounded-xl p-5 sm:p-6 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <h4 className="text-xs sm:text-sm font-bold text-zinc-100 flex items-start gap-2.5 leading-relaxed select-text">
                    <span className="text-zinc-600 font-mono">Q{idx + 1}.</span>
                    <span>{q.question}</span>
                  </h4>
                  {isCorrect ? (
                    <span className="text-[9px] px-2 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-350 font-bold font-mono rounded-md flex items-center gap-1 shrink-0">
                      <CheckCircle2 className="w-3 h-3 text-zinc-300" strokeWidth={1.5} />
                      Correct
                    </span>
                  ) : (
                    <span className="text-[9px] px-2 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-500 font-bold font-mono rounded-md flex items-center gap-1 shrink-0">
                      <XCircle className="w-3 h-3 text-zinc-500" strokeWidth={1.5} />
                      Incorrect
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2 select-text">
                  {q.options.map((opt, optIdx) => {
                    ...
                    let styleClasses = 'border-zinc-850 bg-zinc-950/40 text-zinc-500';
                    let markIcon = null;

                    if (isCorrectOption) {
                      styleClasses = 'border-zinc-700 bg-zinc-900/20 text-zinc-200 font-medium';
                      markIcon = <CheckCircle2 className="w-4 h-4 text-zinc-400 shrink-0" strokeWidth={1.5} />;
                    } else if (isUserSelection && !isCorrect) {
                      styleClasses = 'border-zinc-800 bg-zinc-950 text-zinc-500';
                      markIcon = <XCircle className="w-4 h-4 text-zinc-600 shrink-0" strokeWidth={1.5} />;
                    }
        ...
        <section className="flex flex-col sm:flex-row items-center gap-4 bg-zinc-900/10 border border-zinc-800 p-5 rounded-xl">
          <RefreshCw className="w-7 h-7 text-zinc-500 shrink-0" strokeWidth={1.5} />
          <div className="text-center sm:text-left flex-1">
            <h4 className="text-xs font-bold text-zinc-200">Want to challenge yourself again?</h4>
            <p className="text-[11px] text-zinc-500 mt-1 leading-normal">Start another interactive technical session or check your compatibility details dashboard.</p>
          </div>
          <button
            onClick={() => navigate('/dashboard')}
