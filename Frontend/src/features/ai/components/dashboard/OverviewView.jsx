...
const FEATURE_CARDS = [
  {
    ...
    cta: 'Analyze',
    accent: 'text-cyan-200 bg-cyan-200/10 border-cyan-200/20',
  },
  {
    id: 'mock',
    icon: MessageSquare,
    ...
    cta: 'Practice',
    accent: 'text-fuchsia-200 bg-fuchsia-200/10 border-fuchsia-200/20',
  },
  {
    id: 'quiz',
    icon: ClipboardCheck,
    ...
    cta: 'Quiz',
    accent: 'text-emerald-200 bg-emerald-200/10 border-emerald-200/20',
  },
];
...
const StatTile = ({ label, value, detail, icon: Icon, tone = 'cyan' }) => {
  const toneClasses = {
    ...
    pink: 'text-fuchsia-200 bg-fuchsia-200/10 border-fuchsia-200/20',
    green: 'text-emerald-200 bg-emerald-200/10 border-emerald-200/20',
    ...
    <Card className="p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-3xl font-black tracking-tight text-zinc-50">
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-5">
      <motion.section
        ...
        className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.045] shadow-[0_24px_90px_rgba(0,0,0,0.28)] backdrop-blur-sm"
      >
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1.1fr_0.9fr] lg:p-7">
          <div className="flex min-h-56 flex-col justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-cyan-100">
                <TrendingUp className="h-3.5 w-3.5" strokeWidth={1.7} />
                Preparation cockpit
              </div>
              <h2 className="mt-5 max-w-2xl text-3xl font-black tracking-tight text-zinc-50 sm:text-4xl">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-cyan-200/40 bg-cyan-100 px-4 text-sm font-bold text-zinc-950 shadow-[0_0_30px_rgba(103,232,249,0.16)] hover:bg-white"
              >
                Start analysis
              ...
              <button
                ...
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.055] px-4 text-sm font-bold text-zinc-200 hover:bg-white/[0.09]"
              >
                View history
              </button>
            </div>
          ...
          <div className="grid content-end gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <div className="rounded-2xl border border-white/10 bg-zinc-950/55 p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Milestone progress</p>
                  <p className="mt-2 text-2xl font-black text-zinc-50">{completionPercent}%</p>
                </div>
                <Award className="h-6 w-6 text-amber-200" strokeWidth={1.5} />
              </div>
              <div className="mt-4 h-2 rounded-full bg-white/8">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-cyan-200 via-fuchsia-200 to-emerald-200"
                  style={{ width: `${completionPercent}%` }}
                />
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-zinc-950/55 p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Next best action</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <motion.div variants={itemVariants}>
          <Card className="h-full p-5">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Milestones</p>
                <h3 className="mt-1 text-base font-bold text-zinc-50">Preparation checklist</h3>
              </div>
            ...
            <div className="space-y-2.5">
              {checklist.map((item) => (
                <div key={item.label} className="flex items-start gap-3 rounded-2xl border border-white/8 bg-zinc-950/35 p-3">
                  {item.done ? (
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-200" strokeWidth={1.6} />
                  ) : (
                    <Circle className="mt-0.5 h-4 w-4 shrink-0 text-zinc-600" strokeWidth={1.6} />
                  )}
                  <span className={`text-xs leading-5 ${item.done ? 'text-zinc-500 line-through' : 'font-semibold text-zinc-200'}`}>
                    {item.label}
                  </span>
                </div>
              ))}
        ...
        <motion.div variants={itemVariants}>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Launch tools</h3>
          </div>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <h4 className="text-sm font-bold text-zinc-50">{feature.title}</h4>
                    <p className="mt-2 text-xs leading-5 text-zinc-500">{feature.description}</p>
                  </div>
                  <span className="mt-5 inline-flex items-center gap-1 text-xs font-bold text-zinc-300">
                    {feature.cta}
      ...
      {reports.length > 0 && (
        <motion.section variants={itemVariants}>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Recent reports</h3>
            <button
              type="button"
              onClick={() => onSelectFeature('recent')}
            ...
            {reports.slice(0, 3).map((report) => (
              <Card key={report._id} hover onClick={() => onNavigateReport(report._id)} className="p-4">
                ...
                <div className="mt-5 flex items-end justify-between gap-3">
                  ...
                  <span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-2.5 py-1 text-[10px] font-black text-cyan-100">
