...
const Register = () => {
  ...
  const handleSubmitCredentials = async (e) => {
    ...
    if (!username || !email || !password) {
      setError('Please provide all details to create an account.');
      return;
    }
    setError('');
    setSuccessMsg('');
    ...
    if (res.success && res.otpSent) {
      ...
      setError(res.error || 'Failed to initialize registration.');
    }
  };
  ...
  const handleVerifyOTPCode = async (e) => {
    ...
    setSuccessMsg('');
    setOtpLoading(true);
    const res = await handleVerifyRegisterOTP(email, otpCode);
    setOtpLoading(false);
    if (res.success) {
      navigate('/dashboard');
    } else {
  ...
  if (loading) return <LoadingScreen message="Creating your workspace" />;
        ...
        <div className="my-auto relative z-10 max-w-sm space-y-8">
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-600 dark:text-zinc-400 font-medium leading-relaxed">
              Create a private workspace for resume analysis, mock interview simulations, and technical skill quizzes.
            </p>
          </div>
        ...
        >
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left mb-6">
            ...
            <p className="mt-1.5 text-xs text-slate-500 dark:text-zinc-400 font-medium">
              {otpSent ? 'For security, we sent a code to verify your signup.' : 'Takes less than a minute to get started.'}
            </p>
          </div>
            ...
            {!otpSent ? (
              ...
              >
                <form onSubmit={handleSubmitCredentials} className="space-y-5">
                  <div>
                    <label htmlFor="username" className="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-2">
                      Username
                    </label>
                  ...
                  >
                    <span>{otpLoading ? 'Preparing...' : 'Create Account'}</span>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="123456"
                      className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950/50 text-sm text-center font-mono font-black text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:bg-white dark:focus:bg-zinc-950 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all tracking-[4px]"
                      required
                    />
                  </div>
                </div>
